...
/**
 * @brief 
 * Creates a cmd_node and adds cmd_name to list of
 * arguments (args). Cmd_name must not be null and
 * must be pre-allocated. Also nulls all redirs. Returns 
 * created node or null in case of memory error
 * @param s 
 * @return t_node* 
 */
...
/**
 * @brief Modifies the first element in args' list. 
 * Frees the previous value. New_name 
...
/**
 * @brief 
 * Adds t_blist to the list of redirections. Key 
 * specifies type of redirection (allocates memory
...
/**
 * @brief 
 * Creates and initializes pipe_node. Returns node 
...
/**
 * @brief
 * Creates an initializes error_node. Returns node
 * or null in case of memory error
 * 
 * @param msg 
 * @return t_node* 
